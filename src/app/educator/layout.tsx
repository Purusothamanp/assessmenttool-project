    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--background)' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '3rem', padding: '0 0.75rem' }}>
          <div className="vibrant-gradient-educator" style={{ padding: '0.6rem', borderRadius: '1rem', color: 'white', boxShadow: '0 8px 16px -4px rgba(5, 150, 105, 0.3)' }}>
            <GraduationCap size={24} />
          </div>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 800, letterSpacing: '-0.5px' }}>
            Teach<span style={{ color: 'var(--educator-primary)' }}>Flow</span>
          </h2>
        </div>
            ...
            return (
              <motion.button
                ...
                style={{
                  ...
                  marginBottom: '0.5rem',
                  borderRadius: '1rem',
                  background: isActive ? 'var(--educator-accent)' : 'transparent',
                  color: isActive ? 'var(--educator-primary)' : 'var(--muted-foreground)',
      ...
      <main style={{ flex: 1, marginLeft: '280px', padding: '0', minWidth: 0 }}>
        ...
        <header style={{
          ...
          <div style={{ color: 'var(--muted-foreground)', fontSize: '0.85rem', fontWeight: 500 }}>
             Dashboard / <span style={{ color: 'var(--foreground)', fontWeight: 600 }}>{pathname === '/educator' ? 'Dashboard' : pathname.split('/').pop()?.replace(/^\w/, c => c.toUpperCase())}</span>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <div style={{ textAlign: 'right' }}>
                <p style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--foreground)' }}>{user?.name || 'Instructor'}</p>
                <p style={{ fontSize: '0.65rem', color: 'var(--muted)', fontWeight: 600, textTransform: 'uppercase' }}>{user?.role || 'Educator'}</p>
              </div>
              <div 
                ...
                style={{ 
                  ...
                  fontWeight: 700,
                  fontSize: '0.9rem',
                  boxShadow: '0 6px 12px -3px rgba(5, 150, 105, 0.2)'
                }}
              >
